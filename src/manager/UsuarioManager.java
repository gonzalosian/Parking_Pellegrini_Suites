...
public class UsuarioManager implements IUsuarioManager {
	...
	public List<UsuarioDTO> obtenerTodos() {
		...
		return listaUsuarios.stream().map(m -> new UsuarioDTO(m.getId(), m.getDni(), m.getApellido(), m.getNombre(),
				m.getIdCargo(), m.getUserName())).collect(Collectors.toList());
	}
	...
	public UsuarioDTO validarLogin(String user, String pass) {
		String sql = "SELECT id, apellido, nombre, dni, idCargo, userName, userPass " + "FROM Usuarios "
		...
		try (PreparedStatement stmt = connection.prepareStatement(sql)) {
			...
			if (rs.next()) {
				usuario = new UsuarioDTO(rs.getInt("id"), rs.getInt("dni"), rs.getString("apellido"),
						rs.getString("nombre"), rs.getInt("idCargo"), rs.getString("userName"));
			}
